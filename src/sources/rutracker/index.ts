...
interface RutrackerSource extends Source {
  group: SourceGroup;
}
...
export function parseRows(html: string, groupMap?: Map<number, SourceGroup>): Row[] {
  ...
  for (const tr of body.split(/<tr[\s>]/i).slice(1)) {
    ...
    const stamps = [...tr.matchAll(/data-ts_text="(\d{9,11})"/gi)].map((m) =>
      Number(m[1]),
    );
    const added = stamps
      .reverse()
...
function makeSource(id: SourceId, group: SourceGroup): RutrackerSource {
  return {
    id,
    label: "RuTracker",
    group,
